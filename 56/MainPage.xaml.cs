...
namespace BaseDatos {
    public partial class MainPage : ContentPage {
        ...
        protected override void OnAppearing() {
            ...
            if (nav.NuevoID != null) {
                int idNuevo = nav.NuevoID.Value;

                var nuevo = Registros.FirstOrDefault(v => v.Id == idNuevo);

                if (nuevo != null) {
                    // moverlo a primera posición
                    Registros.Remove(nuevo);
                    Registros.Insert(0, nuevo);
                }

                nav.NuevoID = null; // limpiar marca
                MostrarPagina();
            }
        }

        private async void Adicionar_Clicked(object sender, EventArgs e) {
            await Navigation.PushAsync(new CrearPage());
        }
    }
}